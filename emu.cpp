...
void emu_run() {
    ...
    while (--bail) {
        ...
        if (*address(0xFF02) & 0x80) {
            std::cout << *address(0xFF01) << std::flush;
...
bool emu_step() {
    ...
    if (debug) { print_result(decode); print_reg(); out << "\n"; }
    ...
    if (decode == JUMP || decode == NO_JUMP) {
        if (reg.PC == last1 || reg.PC == last2) {
            if (debug) { out << "SKIP... "; print_addr(last1); print_addr(last2); out << "\n"; }
            debug = false;
        }
...
byte* address_banked(byte bank, uint16_t addr) {
    //if (addr >= BANK_SIZE*2) out << "bank overflow\n"; //debug
...
void print_result(instr_result it) {
    switch(it) {
        case FAIL:   out << "[FAIL]"; break;
        case IGNORE: out << "[ -- ]"; break;
        case NO_JUMP:out << "[NOJP]"; break;
        case JUMP:   out << "[JUMP]"; break;
...
/*void print_masks() {
    for (int i = 0; i < 256; ++i) {
        ...
        if (instr_mask(i, mask_jump))  out << "J";
        else if (instr_mask(i, mask_unique)) out << "U";
        else out << "-";
    }
